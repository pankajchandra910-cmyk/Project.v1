...
import { trekDetailsData } from "../assets/dummy"; // <--- You'll need to create this dummy data
import { Button } from "../component/button";
import { Card, CardContent, CardHeader, CardTitle } from "../component/Card";
import { Badge } from "../component/badge";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "../component/tabs";
import { FAQSection } from "../component/FAQSection"; // Assuming you might add FAQs to trek data
import { ImageWithFallback } from "../component/ImageWithFallback"; // Assuming you have this utility
...
import {
  ...
  Phone,
  Mail,
  ListTodo, // New icon for 'What to Bring' category
  Footprints, // Use Footprints for walking/trekking icons
} from "lucide-react";

// Map for dynamically rendering icons in "What to Bring" or itinerary
const iconMap = {
  Clock: Clock,
  Route: Route,
  Mountain: Mountain,
  ...
  Phone: Phone,
  Mail: Mail,
  ListTodo: ListTodo, // Example for "What to Bring" category
  Walking: Footprints, // Map 'Walking' keyword from data to Footprints icon
};
...
export default function TrekDetailsPage() {
  ...
  useEffect(() => {
    ...
    setError(null);
    if (trekId) {
      // Simulate API call or data fetch from dummy.js
      const data = trekDetailsData[trekId];
      if (data) {
        setTrekData(data);
        setLoading(false);
        setCurrentImageIndex(0); // Reset image index when new trek is loaded
      } else {
  ...
  }, [trekId]); // Re-run effect when trekId changes
  ...
  const handleBookNow = useCallback((id) => {
    // Navigate to a dedicated booking page for treks
    navigate(`/book-item/${id}`);
    // You might also pass selected date or other options via query params
  }, [navigate]);

  const handleHireGuide = useCallback(() => {
    // Navigate to a page to find/contact guides, possibly pre-filtering for this trek's location
    setSelectedDetailType("guide");
    navigate(`/guides?location=${encodeURIComponent(trekData?.location || "")}`);
  }, [navigate, setSelectedDetailType, trekData]);
  ...
  const handleGetDirections = useCallback((lat, lng, name) => {
    const focusId = trekData?.id || "trek-route"; // Use trek ID as focus area
  ...
  const nextImage = useCallback(() => {
    if (trekData && trekData.images && trekData.images.length > 0) {
                    ...
                    <div className="bg-blue-50/80 p-4 rounded-lg shadow-sm">
                      ...
                      {trekData.guide.contact && (
                        <div className="mt-4 flex flex-wrap gap-2 text-xs">
                           <Button variant="outline" size="sm" onClick={() => window.open(`tel:${trekData.guide.contact}`, '_self')}>
                             <Phone className="w-3 h-3 mr-1" /> Call Guide
                           </Button>
                           <Button variant="outline" size="sm" onClick={() => window.open(`mailto:${trekData.guide.email || ''}?subject=Inquiry about ${trekData.name} Trek`, '_self')}>
                        ...
                        {trekData.whatToBring.map((category, index) => (
                          <div key={index} className="space-y-2 p-3 border rounded-lg bg-gray-50/80">
                            <h4 className="font-medium text-primary flex items-center">
                               {/* If you want category-specific icons, you'd add a property to your category object */}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1 p-3 border rounded-md bg-gray-50/80">
                      ...
                      {/* In a real app, this would be a selectable input */}
                    </div>
                  </div>
